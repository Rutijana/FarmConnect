  ...
  return (
    <div style={{ minHeight: '100vh', background: '#EEF2F7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'white', padding: '40px', borderRadius: '16px', width: '400px', boxShadow: '0 8px 30px rgba(0,0,0,0.12)' }}>
        ...
        <h1 style={{ textAlign: 'center', color: '#1A56A0', margin: '0 0 4px', fontSize: '24px', fontWeight: 'bold' }}>FarmConnect Rwanda</h1>
        <p style={{ textAlign: 'center', color: '#888', marginBottom: '28px' }}>Sign in to your account</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input name="phone" type="tel" placeholder="e.g. 0789123456"
          ...
          <input name="password" type="password" placeholder="Your password"
            ...
            style={{ width: '100%', padding: '12px', marginBottom: '24px', border: '1px solid #DDD', borderRadius: '8px', fontSize: '15px', boxSizing: 'border-box' }} required />

          <button type="submit" disabled={loading}
            style={{ width: '100%', padding: '14px', background: '#1A56A0', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>
            {loading ? 'Signing in...' : 'Sign In'}
          </button>
        </form>

        <p style={{ textAlign: 'center', marginTop: '20px', color: '#888', fontSize: '14px' }}>
          No account? <Link to="/register" style={{ color: '#1A56A0' }}>Create account</Link>
        </p>
      </div>
    </div>
  );
